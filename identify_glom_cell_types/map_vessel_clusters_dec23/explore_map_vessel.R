...
for (r in 1:nrow(samples)){
  ...
  spatial <- readRDS(paste0(smp,'_vsmc.RDS'))
...
saveRDS(stmerged,'merged_33_after_vessel.RDS')
#stmerged <- readRDS('merged_33_after_vessel.RDS')
...
for (cl in as.character(0:11)){
  map_prop_data[cl,'In DKD'] <- sum(stmerged@assays$pred.vsmc1.2@data[cl,dkdspots])
  map_prop_data[cl,'In Ref'] <- sum(stmerged@assays$pred.vsmc1.2@data[cl,refspots])
  ...
  map_prop_data[cl,"DKD Rel cl"] <- map_prop_data[cl,'In DKD'] / sum(stmerged@assays$pred.vsmc1.2@data[cl,])
  map_prop_data[cl,"Ref Rel cl"] <- map_prop_data[cl,'In Ref'] / sum(stmerged@assays$pred.vsmc1.2@data[cl,])
}
